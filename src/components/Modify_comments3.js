...
export default class CompD extends React.Component {
    ...
    handleChange = event => {
        this.setState({
            comments_education:event.target.value
        })
    }
    handleSubmit = (event) => {
        event.preventDefault();
        const comment = {
            comments_education: this.state.comments_education
        }
        axios.put(`http://localhost:5000/comments3/${this.props.match.params.id}`, comment)
        .then(res => {
            if (res.status == 200) {
                this.props.history.push('/components/component4')
            }
        })
    ...
    componentDidMount() {
        ...
        axios.get(`http://localhost:5000/comments3/${commentId}`)
        .then((res) => {
            console.log({res});
            this.setState({
                comments_education : res.data.comments_education,
                id: res.data._id 
            })
        })
        .catch(err => alert(err))
    ...
    render() {
        ...
        return (
            ...
            style={{width: 500, height: 30}}
            type='text'
             value={this.state.comments_education}
            name='comments'
